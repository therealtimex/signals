...
2. Load the `realtimex-signals` skill, run `signals-pp-cli health`, and call CRM tools through `signals-pp-cli agent-tools invoke --agent`. Use skill-bundled shell helpers only when the CLI does not cover the operation or is unavailable.
