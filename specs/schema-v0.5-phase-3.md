...
| `calibrateSimulationRun(runId, { observedUntil? })` | §7. Resolves the run's variant → `content_item` → its `content_posts`; computes `actual_metrics` per the §7.2 precedence, window, and aggregation rules from **shared-scope** evidence only (§7.2a); scores with the same recipe as predictions; inserts calibration row. Fails cleanly if the variant is unpublished. |
...
2. **Compute** (`calibrateSimulationRun`, §4.4). The window is `observed_from` = publish time (from the `published_as` edge properties) to `observed_until` = caller horizon (default "now"), enforced on **every** evidence source. Three rules pin down the actuals (Review pass 1):
   ...
   - **(c) Multi-post aggregation.** `content_posts` is one-to-many per content item. Actuals for the run are the element-wise **sum across the item's posts** with per-post values computed under rules (a)/(b) first; `content_post_id` is set iff exactly one post grounded the row (else NULL, with the per-post breakdown in `calibration` JSON) — the honest per-variant comparison, since the prediction was made for the variant, not a single post.
...
5. **Invariant test extension (ships with slice 3.1, before any real grounding code path has users).** Two assertion families, because §8 protects both bytes and *derived values*:
   - **Zero private bytes:** the §6 fixture DB (`local_only` edges + interactions + `properties_private` + a `local_only` persona + allowlist-excluded fields seeded with sentinel strings — email, phone, tags, `platformData`, `syncErrors`) must produce no sentinel through `createSimulationRun` grounding output, `query_simulations` with `includeAgents`/`includeTranscripts`, `record`/`complete` round-trips, calibration rows, and export endpoints. Same harness as the Phase 1/2 invariant, extended.
   - **Value invariance:** seeding additional `local_only` interactions/activities against a published post must leave `actual_metrics`, `actual_score`, `score_error`, and event-derived counts **numerically identical** (§7.2a) — string-scanning alone cannot catch a private event shifting a shared number.
   - **Negative guards:** `createSimulationRun` against a `local_only` launch throws `SimulationScopeError`; a contact whose only persona is `local_only` grounds with `contact_persona_id` NULL and no persona fields in the digest.
...
|---|---|---|---|
| **3.1 Runs & populations** | `simulation_runs` + `simulation_agents` DDL; `SIMULATION_OUTCOMES` registry; `queries/simulations.ts` create/start/list/get with population materialization, launch-scope guard (`SimulationScopeError`, §8.1), persona pinning (§5), `assembleAgentGrounding` allowlist projection (§8.2); `workflow_type` widen (`simulate`, `calibrate` — both here, one code edit); §8.5 invariant-test extension (private-bytes sentinels + negative guards); tools `create_simulation_run`, `query_simulations` | `0013` | — |
| **3.2 Results, completion & projection** | `recordSimulationAgentResults` with transactional run-ownership enforcement (`SimulationAgentOwnershipError`, §4.4) + `completeSimulationRun`; §6 projection rule incl. latest-completed ordering, status interplay, failed/cancelled non-projection; `upsert_variant` description note; tools `record_simulation_results`, `complete_simulation_run`; projection + lifecycle + cross-run-ownership rejection tests | — | 3.1 |
| **3.3 Transcripts & retention** | `simulation_transcripts` DDL (agent-FK only, §4.2); transcript writes through `record_simulation_results`; `includeTranscripts` read flag (agent-join reads); `simulation-transcript-retention` prune job + **typed maintenance dispatch path in the scheduler runner** (§4.2 — the current runner cannot execute template-less jobs) + runner tests + Sync Health report line; byte accounting; §8.4 transcript-export privacy test | `0014` | 3.2 |
| **3.4 Calibration** | `simulation_calibrations` DDL; shared-only engagement-event read (§7.2a, default-safe scope option on the union reader); `calibrateSimulationRun` over existing pipes with precedence/window/multi-post rules (§7.2b–c) and scoring-recipe versioning; calibrate workflow runner (`workflow_type: "calibrate"`); tool `calibrate_simulation_run`; `query_simulations` calibration surfacing; tests: duplicate evidence (snapshot+events), no-snapshot fallback, multiple posts, horizon boundaries, §8.5 value-invariance under seeded `local_only` events | `0015` | 3.2 (parallel with 3.3) |
