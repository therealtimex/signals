...
Execution is **contact-major**: each selected contact runs through X hydration, avatar enrichment, and persona generation before the next contact starts. Hydration can therefore supply identity data immediately to the same contact's avatar and persona steps, while persona work naturally spaces anonymous X traffic between contacts. The run scope preserves operations that must span the batch: authenticated X lookup is prepared in chunks of up to 100, while anonymous hydration lazily reuses one resolver, pacer, breaker, and cooldown until final cleanup. The visible `PROFILE_PIPELINE_MAX_BATCH = 50` limit is the anonymous resolver's run-volume bound; there is no smaller hidden resolution quota. A contact skipped as `x_web_deferred` due to an active cooldown still runs its avatar and persona steps and remains eligible for a later run. The thread still gets one coherent aggregate message per configured step; an `llm`-unavailable abort (§7.3) stops remaining contacts without discarding already-recorded deterministic work.
...
This deterministic first step resolves X identities to public profile data. Identities arrive keyed two ways: by stable numeric `platformUserId` (archive imports), and by handle — agent research, CSV import, and manual entry all produce identities whose `platformUserId` or `platformHandle` is a username like `sama`. Usable OAuth credentials prefer X API v2: numeric IDs go to `GET /2/users?ids=…` and handles to `GET /2/users/by?usernames=…`, each deduplicated and sent in chunks of at most 100. When credentials are absent, the step uses the bounded anonymous-web fallback specified in [`x-anon-web-hydration.md`](./x-anon-web-hydration.md): a dedicated logged-out `signals-x-anon` browser resolves numeric ID to handle, then a credential-free HTTP request parses public profile metadata. A handle-keyed identity already knows what the browser stage would produce, so it skips that stage and fetches the profile directly. It never uses the connected `signals-publish` session.

- Candidates are active X identities with either a numeric `platformUserId` or a resolvable handle (`platformHandle`, else `platformUserId`, matching `/^[A-Za-z0-9_]{1,15}$/`) that have missing profile fields, an archive-derived placeholder contact name, **or** a non-numeric `platformUserId` — an identity with no stable platform ID is itself a hydration gap.
...
- `platformHandle` holds an X username **without** the `@`. The sigil is presentation, added by `formatPlatformHandle` at render time and never persisted; `createIdentity` / `updateIdentity` strip it on write, and migration `0029_normalize_platform_handle_sigil` repaired the `contact_identities` rows that predate that. The column is *not* uniformly bare across platforms, and the enforcement is deliberately narrow: (a) normalization is X-only, so a handle on the other sigil platforms in `PLATFORMS` — `instagram`, `threads`, `tiktok`, `bluesky`, `telegram` — is stored and rendered exactly as given; (b) the LinkedIn writers disagree, `linkedin/mappers.ts` and `linkedin/adapter.ts` storing `/in/<vanity>` where `linkedin/csv-import.ts` stores the bare vanity name; and (c) several writers fall back to a person's display name when no handle exists (`linkedin/mappers.ts:61,114`, `gmail/mappers.ts:92`, `gmail/takeout-import.ts:30`), so the column can legitimately hold a display string. All three are self-consistent with the render path, which leaves non-X handles verbatim.
- A hydrated handle-keyed identity is promoted to the resolved numeric `platformUserId` so later runs take the indexed numeric path. `(platform, platform_user_id)` is globally unique and the owner's own platform account is reserved, so promotion can lose: the hydrated profile fields are kept, `platformData.userIdPromotion = { status: "conflict", resolvedUserId, at, reason }` records why, and the contact outcome carries `detail.userIdConflicts`. The identity stays keyed by its handle and is re-attempted after the 30-day freshness window.
- The two empty cases are distinguishable. `no_x_identity` means the contact has no active X identity at all; `x_identity_unresolved` means it has active X identities carrying neither a numeric ID nor a usable handle. A handle the API reports as unknown is an explicit not-found and caches the 30-day miss like any other.
...
- No OAuth credentials engage the anonymous fallback (unless `webFallback:false` explicitly preserves `x_not_connected`); expired credentials → `x_reauth_required`; rate limit → `x_rate_limited` with `retryAfter`; unavailable API tier → `x_access_restricted`. Anonymous-web outcomes use the bounded `x_web_*` taxonomy in the linked spec. These are skips so avatar/persona work can continue, `x_web_deferred` included: the backlog predicate is needsAvatar/needsPersona/personaStale only, so a contact is never queued *for* hydration. Ending its work bought nothing and cost every avatar it could have cached during the 15-minute cooldown (#436); the deferred count is reported as `hydrationOutcomes.deferred`. Other request/write failures are contact-scoped failures.
