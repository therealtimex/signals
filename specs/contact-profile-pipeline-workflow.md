...
- `platformHandle` stores the **canonical bare identifier** for every platform — an X username, a LinkedIn vanity name, a Gmail address. The `@` on an X handle is presentation and is added by `formatPlatformHandle` at render time, never persisted. Migration `0029_normalize_platform_handle_sigil` strips sigils from `contact_identities` rows written before this was enforced.
