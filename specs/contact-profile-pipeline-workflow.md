...
- `platformHandle` stores a **bare identifier**, not a display string: an X username, a Gmail address, a LinkedIn vanity name. The `@` on an X handle is presentation and is added by `formatPlatformHandle` at render time, never persisted. `createIdentity` / `updateIdentity` normalize on write, so every writer that goes through them upholds this; migration `0029_normalize_platform_handle_sigil` repaired `contact_identities` rows written before it. **Known exception:** the LinkedIn writers disagree with each other — `linkedin/mappers.ts` and `linkedin/adapter.ts` store `/in/<vanity>` while `linkedin/csv-import.ts` stores the bare vanity name. Display is unaffected (non-X handles render verbatim), so this is recorded rather than normalized.
