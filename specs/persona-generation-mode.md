...
- **Dispatch path amendment (#331):** the handoff is submitted once through `POST /cli/send-message` with `requireTerminalDispatch: true` and the persona job ID as `channelTurnId`. RealTimeX PromptInput resolves the thread/default agent, reuses or launches the thread-linked terminal session, and performs provider-aware submission. Signals does not parse or send terminal-agent identity, and persona jobs do not own or terminate the shared session.
...
6. Submit `buildPersonaJobBriefRoutingMessage(...)` once with `dispatchTerminalAgentViaSendMessage({ workspaceSlug, threadSlug, message, channelTurnId: job.id })`. The canonical PromptInput path persists the handoff, resolves the thread/default agent, and reuses or launches the thread-linked runtime before submitting the turn.
...
Session ownership: the shared `Persona Generation` thread owns its PromptInput session. Persona terminal states do not release `rtxRuntimeSessionId`, because multiple jobs may be routed through the same runtime. RealTimeX owns normal session lifecycle; the thread remains the audit trail and is recreated if the user deletes it.
...
**ADR-314-3 (amended by #331): One dedicated RTX thread using canonical PromptInput dispatch.** — Accepted. Context: per-job threads provided isolation but flooded the workspace timeline, while forcing a fresh runtime per job duplicated RealTimeX routing and prevented normal thread-session reuse. Options: (a) retain one thread or runtime per job — rejected after production feedback; (b) share one `Persona Generation` thread while retaining unique jobs, immutable brief files, `jobId`-keyed callbacks, and the job ID as the PromptInput `channelTurnId` — chosen. Consequences: handoffs can interleave in one audit timeline, so each message names the contact, job ID, and brief path and agents are explicitly instructed to ignore prior jobs/messages; Persona never terminates the thread-owned session; first-use thread creation converges under concurrent dispatches and avoids creation when thread presence is unknown.
