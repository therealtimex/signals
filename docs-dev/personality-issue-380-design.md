...
| ADR-380-3 | Legacy-unbound lane follows the **merged** #379 semantics, which are stricter than spec §6.2 prose: an aware (≥1.1.0) skill on an **unbound** workspace may create only **targetless, unaudited** drafts (`variant-use-cases.ts:68`); `targetId` or `audit` → `VALIDATION_ERROR / personality_binding_required`. Its operational sequence is separate from the full lane: omit `targetId` and `personality`, send `audit: null`, persist a `legacy_unbound sketch` label, then stop before approval/materialization/export/publish. | The code is the migration contract #380 is told to follow. The skill must not paper over it, and must never downgrade its declared version to escape the gate (new never-do). |
...
| ADR-380-6 | Every Personality surface the skill renders is a projection of persisted records: binding/status card from `get_writing_context.personality` / `get_personality_binding`; proposal card from `proposals[]` incl. `actions.approvalBlockers`; audit line from the server-stamped `audit.personality`; stale-state card from `PersonalityStatus.detail`. The card uses contact/org IDs where the named view has no names, and labels recovery instructions as skill doctrine rather than persisted state. The skill never reconstructs state from prose and never authors the `core/voice/personality-source-stale` finding (server strips and re-inserts it byte-exactly). | Issue acceptance: "All displayed binding/proposal/audit state comes from persisted server records." |
...
- Proposal card (projection spec §5.6 shape, including workspace, source revisions/profile hash,
  per-file `diff`/`driftDiff`, and preserved `unmanagedBytes`) and binding/stale-state card formats.
...
**`core/approval.md`**: full-lane approval card gains a `Personality` line mapped to the persisted
`get_writing_context.variants[]` entry: `personality.bindingId`, `personalityState`,
`personality.identity.selfContactId`, `personality.identity.representedOrgId`, and nullable
`personality.target.represents.kind`. Legacy-unbound sketches never enter this card.
