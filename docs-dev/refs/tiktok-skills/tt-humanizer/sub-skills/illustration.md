# Sub-skill: Generate an illustration for a TikTok post

Adds an optional image to a draft (feed image, 9:16 cover, or quote-card) and
...
- User says "add an image", "make an illustration", "make a quote-card", or
  "make a cover" for a TikTok post.
...
3. **Apply brand overlay (if profile has it).** Read `../../references/voice-profile.md`
   ...
   r = illustrate("<scene prompt>", kind="story",
