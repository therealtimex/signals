...
VITE_CONFIG="$ROOT/vitest.config.ts"
...
Usage:
  ...
  scripts/qa/run-persona-agent-job-smoke.sh apply --contact-id <id> --response FILE [--meta FILE | --prompt FILE] [--dry-run]

Environment:
  SIGNALS_BASE_URL           Default base URL when prepare did not record one (http://127.0.0.1:3000)
  SIGNALS_AGENT_TOOL_TOKEN   Bearer token when agent-tools API is not localhost-only

Examples:
  scripts/qa/run-persona-agent-job-smoke.sh prepare --contact-id CONTACT_ID --out /tmp/persona-agent-job.txt
  scripts/qa/run-persona-agent-job-smoke.sh verify --response /tmp/persona-response.json
  scripts/qa/run-persona-agent-job-smoke.sh apply --contact-id CONTACT_ID --response /tmp/persona-response.json --prompt /tmp/persona-agent-job.txt
...
cd "$ROOT"
exec npx vite-node --config "$VITE_CONFIG" "$TS_ENTRY" "$@"
