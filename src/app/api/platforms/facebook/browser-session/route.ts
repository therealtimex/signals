...
/**
 * POST /api/platforms/facebook/browser-session
 */
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      case "setup": {
        const result = await openPlatformBrowserSession("facebook");
      ...
      case "validate": {
        const result = await validatePlatformBrowserSession("facebook");
        return NextResponse.json({
...
/**
 * GET /api/platforms/facebook/browser-session
 */
export async function GET() {
  const status = await getPlatformSessionStatus("facebook");
  return NextResponse.json({
...
/**
 * DELETE /api/platforms/facebook/browser-session
 */
export async function DELETE() {
  await disconnectPlatformBrowserSession("facebook");
