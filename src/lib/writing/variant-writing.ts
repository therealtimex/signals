...
import {
  ...
  type VariantWriting,
  type AuthoritativeVariantWritingInput,
  authoritativeVariantWritingInputSchema,
...
import type { WritingAuditPersonality, VariantPersonalitySnapshot } from "@/lib/writing/personality-lineage";
import {
...
export type PersistPersonalityContext = {
  snapshot: VariantPersonalitySnapshot;
  auditPersonality: WritingAuditPersonality | null;
};
...
function findByRequestHash(
  runner: DbRunner,
  launchId: string,
  requestHash: string,
): Variant | undefined {
  return runner.select().from(variants).where(eq(variants.launchId, launchId)).all()
...
function validateCrossDocuments(input: {
  variantId: string;
  body?: string | null;
  writing: AuthoritativeVariantWritingInput;
  ...
  for (const ref of [input.writing.lineage.derivedFromVariantId, input.writing.lineage.adaptedFromVariantId].filter(Boolean) as string[]) {
    if (!runner.select({ id: variants.id }).from(variants).where(eq(variants.id, ref)).get()) throw new AgentToolError("NOT_FOUND", `Lineage variant not found: ${ref}`, { reason: "lineage_ref_not_found" });
  }
  if (input.writing.lineage.adaptedFromContentItemId && !runner.select({ id: contentItems.id }).from(contentItems).where(eq(contentItems.id, input.writing.lineage.adaptedFromContentItemId)).get()) throw new AgentToolError("NOT_FOUND", `Lineage content item not found: ${input.writing.lineage.adaptedFromContentItemId}`, { reason: "lineage_ref_not_found" });
  let targetKind: string | undefined;
  if (input.writing.targetId) {
    const target = resolveTargetById(input.writing.targetId, runner);
  ...
  for (const assetId of input.writing.media?.assetIds ?? []) if (!runner.select({ id: mediaAssets.id }).from(mediaAssets).where(eq(mediaAssets.id, assetId)).get()) throw new AgentToolError("NOT_FOUND", `Media asset not found: ${assetId}`, { reason: "media_asset_not_found" });
...
function deriveLaunchReady(runner: DbRunner, launchId: string, launch: LaunchWriting): void {
  if (!launch.spine || launch.surfaces.length === 0) return;
  const launchRow = runner.select().from(launches).where(eq(launches.id, launchId)).get();
  if (!launchRow || ["live", "completed", "archived"].includes(launchRow.status)) return;
  const rows = runner.select().from(variants).where(eq(variants.launchId, launchId)).all();
  ...
  if (complete) runner.update(launches).set({ status: "ready", updatedAt: Math.floor(Date.now() / 1000) }).where(eq(launches.id, launchId)).run();
}

export function persistWritingVariant(
  input: PersistInput,
  personalityContext?: PersistPersonalityContext,
  runner: DbRunner = db,
): { variant: Variant; created: boolean; lineageEdges: LineageEdgeSummary[] } {
  ...
  const launchRow = runner.select().from(launches).where(eq(launches.id, input.launchId)).get();
  ...
  let existing = input.id ? runner.select().from(variants).where(eq(variants.id, input.id)).get() : undefined;
  ...
  if (!existing && !input.id) existing = findByRequestHash(runner, input.launchId, generationResult.data.requestHash);
  ...
  const selector = writingResult.data.personality;
  if (personalityContext) {
    if (!selector || selector.bindingId !== personalityContext.snapshot.bindingId) {
      fail("Personality selector does not match the authoritative binding", "personality_binding_stale", ["metadata", "writing", "personality", "bindingId"]);
    }
  } else if (selector && selector !== null || previous?.personality) {
    fail("Personality-bound writing must use the guarded application service", "guarded_writer_required", ["metadata", "writing", "personality"]);
  }
  const authoritativeWriting: AuthoritativeVariantWritingInput = authoritativeVariantWritingInputSchema.parse({
    ...writingResult.data,
    ...(personalityContext
      ? { personality: personalityContext.snapshot }
      : selector === null
        ? { personality: null }
        : {}),
  });
  if (existing) {
    const materializedEdge = existing.contentItemId
      ? undefined
      : runner.select().from(graphEdges).where(and(
  ...
  if (authoritativeWriting.audit) {
    const expectedHard = deriveHard({ units: authoritativeWriting.units, media: authoritativeWriting.media, limit: hardLimit(authoritativeWriting.surface) });
    if (!sameHard(authoritativeWriting.audit.hard, expectedHard)) fail("Audit hard counts do not match variant", "audit_hard_mismatch", ["metadata", "writing", "audit", "hard"]);
    if (JSON.stringify(authoritativeWriting.audit.overlay) !== JSON.stringify(authoritativeWriting.overlay) || JSON.stringify(authoritativeWriting.audit.core) !== JSON.stringify(authoritativeWriting.core)) fail("Audit ruleset does not match variant", "audit_overlay_mismatch", ["metadata", "writing", "audit"]);
    const sourceStale = personalityContext?.auditPersonality?.statusAtAudit === "source_stale";
    const findings = [
      ...authoritativeWriting.audit.findings.filter(
        (finding) => finding.code !== PERSONALITY_SOURCE_STALE_FINDING_CODE,
      ),
      ...(sourceStale && personalityContext?.auditPersonality
        ? [personalitySourceStaleFinding(personalityContext.auditPersonality)]
        : []),
    ];
    const provisional = {
      ...authoritativeWriting.audit,
      findings,
      ...(personalityContext ? { personality: personalityContext.auditPersonality } : {}),
      id: previous?.audit?.inputHash === inputHash ? previous.audit.id : newWritingId("aud"),
      variantId,
      inputHash,
      hard: expectedHard,
    };
    ...
    const expectedVoiceStatus = authoritativeWriting.voicePrecedence === "rules_first"
      ? "rules_first"
      : authoritativeWriting.voiceProfile
        ? "applied"
        : "none";
    ...
    if (authoritativeWriting.voicePrecedence === "rules_first" && candidate.findings.some((finding) => "skippedForVoice" in finding && finding.skippedForVoice)) fail("rules_first cannot skip findings for voice", "audit_voice_mismatch", ["metadata", "writing", "audit", "findings"]);
    ...
    if (!sourceStale && candidate.verdict !== verdict) fail("Submitted audit verdict does not match server verdict", "audit_verdict_mismatch", ["metadata", "writing", "audit", "verdict"]);
  ...
  const approval = input.status === "rejected"
    ...
    : approvalFor({
        ...
        ...(previous?.audit?.personality
          ...
          ? { staleReason: "personality_source_stale" as const }
          : {}),
      });
...
export function revokeVariantApproval(variantId: string, reason: ApprovalState["revokedReason"], note?: string) {
  ...
  return db.transaction((tx) => {
    const result = revokeWritingVariantWithRunner(tx, { variant, reason, note });
    if (result.blocked) throw new AgentToolError("CONFLICT", "Cannot revoke a variant in the publish lane");
    return variantWritingSchema.parse(object(tx.select().from(variants).where(eq(variants.id, variantId)).get()!.metadata).writing).approval;
  });
}
...
export function revokeVariantsForSpineChange(launchId: string): string[] {
  return db.transaction((tx) => {
    ...
    for (const variant of tx.select().from(variants).where(eq(variants.launchId, launchId)).all()) {
