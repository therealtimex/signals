...
3. For each candidate thread:
   ...
   c. On X, publish with \`.claude/skills/signals-publish/scripts/x-reply.cjs\` (single-pass insertText of the entire reply, a fail-closed pre-submit editor snapshot check with one re-inject, then a confirmed new reply URL). Never type paragraphs separately. Then apply a randomized 20s–45s salted sleep delay between posts to maintain safe human cadence.
