...
export function acquireSessionLease(
  ...
  return db.transaction((tx) => acquireSessionLeaseWithRunner(tx, connectionId, input));
}

export function acquireSessionLeaseWithRunner(
  runner: DbRunner,
  connectionId: string,
  input: {
    holder: string;
  ...
  runner.insert(browserSessionLeases)
    .values(values)
    .onConflictDoUpdate({
...
export function renewSessionLease(
  ...
  return db.transaction((tx) => renewSessionLeaseWithRunner(tx, leaseId, ttlSeconds));
}

export function renewSessionLeaseWithRunner(
  runner: DbRunner,
  leaseId: string,
  ttlSeconds?: number,
): BrowserSessionLease {
  ...
  runner.update(browserSessionLeases)
...
export function releaseSessionLease(leaseId: string): void {
  releaseSessionLeaseWithRunner(db, leaseId);
}

export function releaseSessionLeaseWithRunner(
  runner: DbRunner,
  leaseId: string,
): void {
  const result = runner
