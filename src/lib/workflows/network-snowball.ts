...
export function buildNetworkSnowballBriefSection(input: {
  ...
  const lines = [
    ...
    "    - Terminate Agent Session: Call complete_workflow_run (step 10) when finished. Signals stops running browser sessions immediately and keeps the linked terminal session attached for review/resume — do not send further messages in this thread after completion.",
  ];
