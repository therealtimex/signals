...
export function buildNetworkSnowballBriefSection(input: {
  ...
  const lines = [
    ...
    "    - Resolver is the last resort, not the easy path: `https://unavatar.io/...` is capped near 50 requests/day for the whole install, so an avatar sourced there may not render until it has been cached locally. Use it only when the scrape genuinely fails, and never leave the field blank instead — a resolver URL is still better than nothing. Use `https://unavatar.io/linkedin/user:{slug}` for a person (`linkedin.com/in/{slug}`) and `https://unavatar.io/linkedin/company:{slug}` for an organization page (`linkedin.com/company/{slug}`); the `user:` namespace 404s for a company slug and vice versa, so pick by the profile URL you actually visited.",
