...
export function buildNetworkSnowballBriefSection(input: {
  ...
  const lines = [
    ...
    "S4. Avatar Enrichment (downstream of identity attestation): For LinkedIn, use the `avatarUrl` returned by `attest_snowball_linkedin_identity`. Signals extracts it server-side from the attested top card (`.pv-top-card-profile-picture__image` / `main [data-view-name=\"profile-card\"]`) and rejects navbar/session-viewer photos (`shrink_100_100`, `shrink_50_50`, `.global-nav`). Never scrape the first `img[src*=profile-displayphoto]` on the page.",
