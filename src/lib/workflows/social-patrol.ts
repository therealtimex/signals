...
export function buildSocialPatrolBriefSection(input: {
  ...
  const lines = [
    ...
    "    X composer contract (mandatory on x.com): never type a reply line-by-line, never press Enter between paragraphs, and never call insertParagraph, iterative insertText, or document.execCommand insert/selectAll/delete. Inject the entire drafted reply with one CDP Input.insertText (`agent-browser keyboard inserttext`) via `.claude/skills/signals-publish/scripts/x-reply.cjs` (the same single-pass helper used by `x-publish.cjs`). Before `[data-testid=\"tweetButtonInline\"]` or modal `[data-testid=\"tweetButton\"]` is clicked, that script requires Draft-owned leaf text (and EditorState when readable) whose paragraph structure matches the draft; if they diverge, it re-injects once and rechecks, then aborts without submitting. After a Reply click it confirms the full drafted text on the acting profile's replies timeline and returns that reply's `platformPostId` / `platformUrl`. If confirmation fails (`verify_uncertain`), do not click Reply again.",
