...
export function buildAgentWorkflowBrief(input: {
  ...
  const sections = [
    ...
    `6. Discover tools: GET ${input.signalsBaseUrl}/api/agent-tools`,
    `7. For single-record edits, invoke tools (${tools}) via POST ${input.signalsBaseUrl}/api/agent-tools/invoke with JSON { \"tool\": \"...\", \"input\": { ... } }.`,
    "8. Perform web search and browser work in RealTimeX (not via agent-tools).",
    "9. Report a concise summary in this thread when finished (import JSON summary is suitable).",
    "",
