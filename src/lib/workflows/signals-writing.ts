...
export function buildWritingBriefSection(input: {
  ...
  return [
    ...
    "3. Approved voice profiles and validated variant persistence become available after #350; do not invent either contract in this run.",
    "4. Draft one native artifact per surface. Use create_content_draft with a stable idempotencyKey, then update_content_draft with expectedUpdatedAt for bounded revisions.",
    "5. Approval/materialization becomes available after #350. Draft creation itself never authorizes publishing.",
    "6. Call complete_workflow_run with a concise result and any missing surfaces or blockers.",
    "Hard rules:",
    "- Use only manifest-backed agent tools; do not call removed in-process helpers or lib wrappers.",
    "- Do not write a content item directly for a variant; materialization owns that boundary once available.",
