...
export function buildWritingBriefSection(input: {
  ...
  return [
    ...
    ...capabilityRows,
    "Tool sequence:",
    "1. Read the workspace Personality files (IDENTITY.md, SOUL.md, VOICE.md, BRAND.md) first. Treat get_writing_context.personality as the binding gate; bound work submits only its active bindingId.",
    ...
    "6. Persist by the lane returned in `get_writing_context.personality.status`:",
    ...
    "8. In the `bound`/`source_stale` full lane only, render the persisted approval card after audit and precheck, wait for explicit user approval, then call `materialize_variant`. Include the real approval evidence; do not manufacture it. Use `revoke_variant_approval` when the user withdraws approval.",
