...
export function buildWritingBriefSection(input: {
  ...
  return [
    ...
    "5. Draft and audit one platform-native artifact per surface, then call upsert_variant with generationMetadata.kind=signals-writing and the complete metadata.writing contract. Revisions reuse the returned variant id.",
    "6. Call materialize_variant only after the server-validated audit passes. Under explicit policy include user approval evidence; do not manufacture it. Use revoke_variant_approval when the user withdraws approval.",
    "7. Call complete_workflow_run with the variant/content IDs and any missing surfaces or blockers.",
    "Hard rules:",
