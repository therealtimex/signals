...
export type ResolvedContactCompany =
  | { company: string | null; touched: true }
  | { touched: false };
...
export function resolveContactCompanyFields(input: ContactOrgInput): ResolvedContactCompany | { error: string } {
  ...
  if (input.orgId) {
    ...
    return { company: org.name, touched: true };
  }
...
export function syncContactCompanyFromContact(
  ...
  syncContactCompanyGraph(contactId, company, title, source);
}
