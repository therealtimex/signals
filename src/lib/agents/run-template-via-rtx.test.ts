...
describe("runTemplateViaRtx health preflight", () => {
  ...
  it("revokes the writing scope when terminal dispatch is rejected", async () => {
    // The plaintext token is already on disk in the brief by the time a send can be rejected, so a
    // live hash would let a filesystem reader mint a scope for a dispatch that never happened.
    const template = createTemplate({
      name: "Contact Relationship Nurture",
      templateType: "nurture",
      status: "active",
    ...
    expect(result.success).toBe(false);
    if (result.success || !result.workflowRunId) throw new Error("expected a rejected dispatch");
    expect(getWorkflowRun(result.workflowRunId)).toMatchObject({ status: "failed" });

    // The brief was already written, so the plaintext token is recoverable from disk.
    const brief = readFileSync(
      join(storageDir, "working-data/signals/workflow-runs", result.workflowRunId, "brief.md"),
      "utf8",
    );
    ...
    // The hash is gone, so that token now resolves to nothing and can stamp no scope.
    expect(JSON.parse(getWorkflowRun(result.workflowRunId)?.config ?? "{}"))
      .not.toHaveProperty(WRITING_SCOPE_TOKEN_CONFIG_KEY);
    expect(resolveComposedRunAuthorityByToken(db, token!)).toBeNull();

    const launchesBefore = db.select().from(launches).all().length;
    await expect(invokeAgentTool("upsert_launch", {
      name: "Launch from a rejected dispatch",
      writingScopeToken: token,
    })).rejects.toMatchObject({ details: { reason: "writing_scope_token_invalid" } });
    expect(db.select().from(launches).all()).toHaveLength(launchesBefore);
  });

  it("keeps the capability when dispatch was accepted but later bookkeeping fails", async () => {
    // The revocation guard is `!dispatchAccepted`: an agent that really was handed the brief must
    // not lose its capability because post-dispatch bookkeeping threw.
    const template = createTemplate({
      name: "Contact Relationship Nurture",
      templateType: "nurture",
      status: "active",
    ...
    vi.spyOn(workflowTemplates, "updateTemplate").mockImplementation(() => {
      throw new Error("bookkeeping exploded after dispatch");
    });
    ...
    expect(result.success).toBe(false);
    if (result.success || !result.workflowRunId) throw new Error("expected a bookkeeping failure");
    ...
    const token = /writingScopeToken: "([^"]+)"/.exec(brief)?.[1]!;
    expect(resolveComposedRunAuthorityByToken(db, token)).toMatchObject({
      workflowRunId: result.workflowRunId,
    });
  });
